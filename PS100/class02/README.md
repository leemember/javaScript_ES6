...
scores라는 변수에 학생들의 점수를 객체로 배열에 넣는다. 그리고 scores 를 sort함수를 사용하여 function 매개변수 인자로 오름차순을 해준다. 그리고 콘솔로 scores를 찍었을 시, 오름차순으로 숫자들이 정렬되는 것을 확인해 볼 수 있다. 그리고 이제 top3 학생들을 걸러내주는 로직을 작성할 것인데, `let top3 = [];` 이렇게 top3 변수를 배열에 초기화를 시켜준다. 그리고 `while`문으로 조건문을 만들어주는데 `(top3.length < 4)` 는 top3 학생들을 1등부터 3등까지 걸러내주기 위한 조건문이다. 그리고 value라는 변수를 만들어 score에 `pop()` 함수를 해주는데, 여기서 `pop()` 함수는 **scores의 배열에 가장 끝에 있는 원소를 밖으로 빼내주는 역할을 한다.** 그리고 `if` 조건문을 사용하여 pop으로 빼낸 원소들을 value 변수에 넣고 value 변수는 다시 top3 배열에 `push`함수를 이용하여 **score에 집어 넣어주도록 한다.**
